...
def choose_poketer(available_poketers, is_cpu):
    if is_cpu:
        ...
        print("Du ska nu få välja en Poketer1. Dessa Pokteter är tillgängliga:")
        if len(available_poketers) > 0:
            for idx, poketer in enumerate(available_poketers):
                print(idx + 1, poketer.name)

            poketer_choice = take_integer_input(f"Vilken Poketer1 väljer du? (1-{len(available_poketers)}): ",
